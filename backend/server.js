// ── Catch all crashes and log them clearly ──
...
// ─────────────────────────────────────────────────────────
//  DATABASE  (promise pool — fixes the .then()/.catch() crash)
// ─────────────────────────────────────────────────────────
...
//  Railway blocks SMTP port 587 → use HTTP API instead
//
...
console.log('Email:', emailEnabled ? 'ENABLED via Brevo HTTP API' : 'DISABLED — set BREVO_API_KEY and SENDER_EMAIL');
...
async function sendEmail(to, subject, html) {
  ...
  try {
    ...
    console.log('Email sent to', to, '| messageId:', resp.data?.messageId);
  } catch (e) {
      ...
      <div style="padding:30px;background:#f9f9f9">
        ...
        <table style="width:100%;border-collapse:collapse">
          ...
          <tr style="background:#c4a050">
            <td style="padding:10px;color:#000;font-weight:bold">Your Payout</td>
...
app.get('/api/auth/bookings', authMiddleware, async (req, res) => {
  try {
    const [data] = await db.query(`
      SELECT b.*,
        COALESCE(r.type,'External Room') AS type, COALESCE(r.room_number,'EXT') AS room_number,
        COALESCE(r.image_url,'') AS image_url, COALESCE(h.name,b.external_ref) AS hotel_name,
        COALESCE(h.city,'') AS city, COALESCE(h.country,'') AS country
      FROM bookings b
      LEFT JOIN rooms r ON b.room_id=r.id LEFT JOIN hotels h ON b.hotel_id=h.id
...
app.get('/api/hotels', async (req, res) => {
  const { q } = req.query;
  try {
    let sql = `SELECT h.*, COALESCE(AVG(rv.rating),0) AS avg_rating, COUNT(rv.id) AS review_count
               FROM hotels h LEFT JOIN reviews rv ON rv.hotel_id=h.id WHERE h.status='active'`;
    const params = [];
    if (q) { sql += ' AND (h.city LIKE ? OR h.country LIKE ? OR h.name LIKE ?)'; const t=`%${q}%`; params.push(t,t,t); }
...
app.post('/api/hotels/apply', async (req, res) => {
  ...
  try {
    ...
    if (SENDER_EMAIL) {
      sendEmail(SENDER_EMAIL, `New Partner: ${hotel_name}`,
        `<div style="font-family:Arial,sans-serif;padding:20px"><h2>New Partner Application</h2>
        <p><b>Hotel:</b> ${hotel_name}</p><p><b>Location:</b> ${city}, ${country}</p>
        <p><b>Contact:</b> ${contact_name}</p><p><b>Email:</b> ${email}</p>
        <p><b>Phone:</b> ${phone||'N/A'}</p><p><b>Message:</b> ${message||'N/A'}</p></div>`
      );
    }
...
app.post('/api/contact', async (req, res) => {
  ...
  sendEmail(
    ...
    `<div style="font-family:Arial,sans-serif;max-width:600px">
      <div style="background:#0a0e17;padding:20px;text-align:center"><h1 style="color:#c4a050;margin:0">AMARYA</h1></div>
      <div style="padding:24px;background:#f9f9f9">
        <h2>New Contact Message</h2>
        <p><b>Name:</b> ${name}</p><p><b>Email:</b> ${email}</p>
        <p><b>Subject:</b> ${subject||'N/A'}</p><p><b>Message:</b> ${message}</p>
  ...
  sendEmail(
    email,
    'Message received — Amarya',
    `<div style="font-family:Arial,sans-serif;max-width:600px">
      <div style="background:#0a0e17;padding:20px;text-align:center"><h1 style="color:#c4a050;margin:0">AMARYA</h1></div>
      <div style="padding:24px;background:#f9f9f9">
        <h2>Hi ${name},</h2>
        <p>We received your message and will reply within 24 hours.</p>
        <p style="color:#888;font-style:italic">"${message}"</p>
        <p>Best,<br><b>The Amarya Team</b></p>
      </div>
    </div>`
  );
  ...
  try {
    const [data] = await db.query(
      `SELECT COUNT(*) AS total, ROUND(AVG(rating),1) AS average,
       ...
       SUM(rating=2) AS two_star, SUM(rating=1) AS one_star FROM reviews WHERE hotel_id=?`,
      [req.params.id]
    );
    res.json(data[0]);
  } catch (err) { res.status(500).json({ error: err.message }); }
...
app.post('/api/bookings', optionalAuth, async (req, res) => {
  const { room_id, customer_name, customer_email, customer_phone,
          check_in, check_out, guests, special_requests,
          source, offer_id, hotel_name: ext_hotel_name, price_usd } = req.body;
  ...
  if (source === 'amadeus' && offer_id) {
    ...
    } catch (e) { console.error('Amadeus DB insert:', e.message); }
  ...
  try {
    ...
    const rec = { id:bookingNum, customer_name, customer_email, customer_phone, check_in, check_out, nights, guests:guests||1, special_requests, hotel_payout, commission_amount };
        ...
        <div style="padding:30px;background:#f9f9f9">
          ...
          <table style="width:100%;border-collapse:collapse;margin:16px 0">
            ${[['Booking #',bookingNum],['Hotel',room.hotel_name],['Room',`${room.type} #${room.room_number}`],['Check In',check_in],['Check Out',check_out],['Nights',nights],['Guests',guests||1],['Total',`GHS ${total_price.toLocaleString()}`]]
              .map(([l,v],i)=>`<tr style="background:${i%2?'#fff':'#f5f5f5'}"><td style="padding:9px;color:#666;border:1px solid #eee">${l}</td><td style="padding:9px;font-weight:bold;border:1px solid #eee">${v}</td></tr>`).join('')}
          </table>
          <p style="color:#888">Thank you for choosing Amarya.</p>
        </div>
        <div style="background:#0a0e17;padding:16px;text-align:center"><p style="color:#c4a050;margin:0">amarya.netlify.app</p></div>
      </div>`
...
app.get('/api/bookings', async (req, res) => {
  try {
    const [data] = await db.query(`
      SELECT b.*, COALESCE(r.type,'External Room') AS type, COALESCE(r.room_number,'EXT') AS room_number,
             COALESCE(r.image_url,'') AS image_url, COALESCE(h.name,b.external_ref) AS hotel_name,
             COALESCE(h.city,'') AS city, COALESCE(h.country,'') AS country
      FROM bookings b LEFT JOIN rooms r ON b.room_id=r.id LEFT JOIN hotels h ON b.hotel_id=h.id
