...
async function sendEmail(to, subject, html) {
  ...
  try {
    const resp = await axios.post('https://api.brevo.com/v3/smtp/email',
      { sender: { name:'Amarya Hotels', email:SENDER_EMAIL }, to:[{email:to}], subject, htmlContent:html },
...
async function notifyHotel(hotel, booking, room) {
  ...
  const html = `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto"><div style="background:#0a0e17;padding:30px;text-align:center"><h1 style="color:#c4a050;letter-spacing:4px;margin:0">AMARYA</h1></div><div style="padding:30px;background:#f9f9f9"><h2>New Booking Received</h2><table style="width:100%;border-collapse:collapse">${rows}<tr style="background:#c4a050"><td style="padding:10px;color:#000;font-weight:bold">Your Payout</td><td style="padding:10px;color:#000;font-weight:bold">GHS ${Number(booking.hotel_payout).toLocaleString()}</td></tr></table></div></div>`;
